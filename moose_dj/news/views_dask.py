...
async def start_task(request):
    ...
    for t in range(NUM_FAKE_TASKS):
        ...
        work_list.append({"address": f"foo--{t}@example.com", "message": f"Mail task: {task_id}"})
...
async def get_task_status(request, task_id):
    ...
    if futures:
        if not all(future.done() for future in futures):
            ...
            progress = 100
            return JsonResponse({"task_id": task_id, "status": "completed", "progress": progress, "results": results})
    else:
